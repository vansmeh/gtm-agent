...
def test_role_queries_include_snippet_sources_and_stop_at_ten() -> None:
    queries = role_queries("Ada Lovelace", "Northwind")
    assert len(queries) == 10
    assert any(query.startswith("site:linkedin.com/in") for query in queries)
    assert any(query.startswith("site:github.com") for query in queries)
